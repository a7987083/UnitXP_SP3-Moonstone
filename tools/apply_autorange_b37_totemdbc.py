...
bool findTotemSummon(const std::uint32_t spellId,
    ...
    // Parent spells can trigger a child spell that actually performs the summon.
    // Follow only explicit SPELL_EFFECT_TRIGGER_SPELL edges, matching the resolver's
    // existing bounded recursive design and avoiding a global DBC scan.
    for (std::uint32_t i = 0u; i < 3u; ++i) {
        const std::uint32_t effect = gCache.spell.u32(r, 61u + i);
...
std::string totemInfo(const unsigned int spellId) {
    ...
    if (!ensureCache()) {
        return std::string("E|TOTEM_DBC|") + gCache.error;
    }
    ...
    if (lifeMs == 0u && candidate.summonSpell != spellId)
        lifeMs = durationMs(spellId);
