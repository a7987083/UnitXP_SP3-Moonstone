...
NSString *ZNRuntimeArgumentControlTypeName(ZNRuntimeArgumentControlType type) {
    switch (type) {
        case ZNRuntimeArgumentControlTypeSwitch: return @"开关";
        case ZNRuntimeArgumentControlTypeButton: return @"按钮";
        case ZNRuntimeArgumentControlTypeNumber: return @"数值";
        case ZNRuntimeArgumentControlTypeSlider: return @"滑块";
        default: return @"固定";
    }
}
NSString *ZNRuntimeArgumentControlTypeKey(ZNRuntimeArgumentControlType type) {
    switch (type) {
        case ZNRuntimeArgumentControlTypeSwitch: return @"switch";
        case ZNRuntimeArgumentControlTypeButton: return @"button";
        case ZNRuntimeArgumentControlTypeNumber: return @"number";
        case ZNRuntimeArgumentControlTypeSlider: return @"slider";
        default: return @"fixed";
    }
}
ZNRuntimeArgumentControlType ZNRuntimeArgumentControlTypeFromKey(NSString *key) {
    NSString *k = (key ?: @"").lowercaseString;
    if ([k isEqualToString:@"switch"]) return ZNRuntimeArgumentControlTypeSwitch;
    if ([k isEqualToString:@"button"]) return ZNRuntimeArgumentControlTypeButton;
    if ([k isEqualToString:@"number"]) return ZNRuntimeArgumentControlTypeNumber;
    if ([k isEqualToString:@"slider"]) return ZNRuntimeArgumentControlTypeSlider;
    return ZNRuntimeArgumentControlTypeFixed;
}

static NSArray<NSDictionary<NSString *, id> *> *ZNRMADefaultConfigs(NSUInteger count) {
    NSMutableArray *items = [NSMutableArray arrayWithCapacity:count];
    for (NSUInteger i = 0; i < count; i++) {
        [items addObject:@{@"enabled": @NO, @"type": @"fixed", @"min": @0, @"max": @100, @"step": @1}];
    }
    return [items copy];
}
...
- (instancetype)init {
    ...
    _title=@""; _group=@"Runtime Methods"; _assembly=@"Assembly-CSharp.dll"; _namespaceName=@""; _className=@""; _methodName=@"";
    _argumentValues=@[]; _parameterTypeNames=@[]; _signatureAvailable=NO; _argumentControlConfigs=@[]; _immediateChain=@{};
    return self;
}
...
- (NSString *)canonicalIdentity {
    if (self.signatureAvailable && self.parameterTypeNames.count == self.argumentCount) return ZNIL2CPPFullMethodIdentity(self.assembly?:@"",self.namespaceName?:@"",self.className?:@"",self.methodName?:@"",self.parameterTypeNames?:@[]);
    return self.legacyCanonicalIdentity;
}
- (id)copyWithZone:(NSZone *)zone {
    ZNRuntimeMethodAction *copy=[[[self class] allocWithZone:zone] init];
    copy.actionID=self.actionID; copy.title=self.title; copy.group=self.group; copy.assembly=self.assembly; copy.namespaceName=self.namespaceName; copy.className=self.className; copy.methodName=self.methodName;
    copy.argumentCount=self.argumentCount; copy.argumentValues=self.argumentValues?:@[]; copy.parameterTypeNames=self.parameterTypeNames?:@[]; copy.signatureAvailable=self.signatureAvailable;
...
+ (instancetype)sharedStore { static ZNRuntimeActionStore *store; static dispatch_once_t onceToken; dispatch_once(&onceToken, ^{store=[ZNRuntimeActionStore new];}); return store; }
...
- (ZNRuntimeMethodAction *)addMethodCandidate:(NSDictionary<NSString *,id> *)candidate title:(NSString *)title argumentValues:(NSArray<NSString *> *)argumentValues error:(NSString **)error {
    ...
    if(!className.length||!methodName.length||argc<0){if(error)*error=@"方法身份不完整，无法创建 Runtime Method Call";return nil;}
    if((NSUInteger)argc>ZN_RUNTIME_ACTION_MAX_ARGUMENTS){if(error)*error=[NSString stringWithFormat:@"参数数量超过上限 %u",ZN_RUNTIME_ACTION_MAX_ARGUMENTS];return nil;}
    NSArray<NSString *> *values=argumentValues?:@[]; if(argc==0)values=@[];
    if(argc>0&&values.count!=(NSUInteger)argc){if(error)*error=[NSString stringWithFormat:@"/%ld 方法必须提供 %ld 个参数值",(long)argc,(long)argc];return nil;}
    ...
    [[ZNRuntimeLogger sharedLogger] log:[NSString stringWithFormat:@"[runtime-method-call] authoring add id=%u %@ args=%@",action.actionID,action.canonicalIdentity,action.argumentValues]]; return [action copy];
}
...
- (BOOL)updateArgumentControlConfigs:(NSArray<NSDictionary<NSString *,id> *> *)configs atIndex:(NSUInteger)index error:(NSString **)error { @synchronized(self){if(index>=self.mutableActions.count){if(error)*error=@"Runtime Method Call 索引已失效";return NO;}ZNRuntimeMethodAction *action=self.mutableActions[index];if(configs.count!=action.argumentCount){if(error)*error=@"参数控件配置数量必须等于 argc";return NO;}NSMutableArray *clean=[NSMutableArray arrayWithCapacity:configs.count];for(NSDictionary *raw in configs){BOOL enabled=[raw[@"enabled"] boolValue];ZNRuntimeArgumentControlType type=enabled?ZNRuntimeArgumentControlTypeFromKey(raw[@"type"]):ZNRuntimeArgumentControlTypeFixed;[clean addObject:@{@"enabled":@(enabled),@"type":ZNRuntimeArgumentControlTypeKey(type),@"min":raw[@"min"]?:@0,@"max":raw[@"max"]?:@100,@"step":raw[@"step"]?:@1}];}action.argumentControlConfigs=[clean copy];return YES;} }
