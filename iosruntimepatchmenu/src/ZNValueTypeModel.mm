...
NSString *ZNValueTypeKey(ZNValueType type) {
    return ZNValueTypeName(type).lowercaseString;
}
...
BOOL ZNValueTypeIsInteger(ZNValueType type) {
    return type == ZNValueTypeI32 || type == ZNValueTypeU32 || type == ZNValueTypeI64 || type == ZNValueTypeU64;
}

BOOL ZNValueTypeIsFloating(ZNValueType type) {
    return type == ZNValueTypeF32 || type == ZNValueTypeF64;
}
...
NSDictionary<NSString *, NSNumber *> *ZNDefaultRangeForValueType(ZNValueType type, BOOL slider) {
    if (slider) {
        // Product rule: sliders are integer-stepped UI controls. F32/F64 still
        // receive 1.0, 2.0 ... at the backend. Static FMOV may extend max to 31.
        return @{@"default": @1, @"min": @1, @"max": @10, @"step": @1};
    }
    switch (type) {
        case ZNValueTypeI32:
            return @{@"default": @1, @"min": @(INT32_MIN), @"max": @(INT32_MAX), @"step": @1};
        case ZNValueTypeU32:
            return @{@"default": @1, @"min": @0, @"max": @((uint64_t)UINT32_MAX), @"step": @1};
        case ZNValueTypeI64:
            return @{@"default": @1, @"min": @((long long)INT64_MIN), @"max": @((long long)INT64_MAX), @"step": @1};
        case ZNValueTypeU64:
            // NSNumber can represent U64 exactly as unsignedLongLong; UI text is
            // canonicalized without a double round-trip.
            return @{@"default": @1, @"min": @0, @"max": @((unsigned long long)UINT64_MAX), @"step": @1};
        case ZNValueTypeF32:
        case ZNValueTypeF64:
            return @{@"default": @1, @"min": @(-FLT_MAX), @"max": @(FLT_MAX), @"step": @1};
        case ZNValueTypeAuto:
        default:
            return @{@"default": @1, @"min": @0, @"max": @(INT32_MAX), @"step": @1};
    }
}

static BOOL ZNParseSigned(NSString *text, long long *value) {
    NSScanner *s = [NSScanner scannerWithString:text ?: @""];
    long long v = 0;
    if (![s scanLongLong:&v] || !s.isAtEnd) return NO;
    if (value) *value = v;
    return YES;
}
...
static BOOL ZNParseUnsigned(NSString *text, unsigned long long *value) {
    ...
    NSScanner *s = [NSScanner scannerWithString:trim];
    unsigned long long v = 0;
    if (![s scanUnsignedLongLong:&v] || !s.isAtEnd) return NO;
    if (value) *value = v;
    return YES;
}

NSString *ZNCanonicalValueString(NSString *input,
                                 ZNValueType type,
                                 NSNumber *minValue,
                                 NSNumber *maxValue,
                                 NSNumber *stepValue,
    ...
    if (type == ZNValueTypeI32 || type == ZNValueTypeI64) {
        long long v = 0;
        if (!ZNParseSigned(text, &v)) { if (error) *error = @"不是有效有符号整数"; return nil; }
        long long lo = type == ZNValueTypeI32 ? INT32_MIN : INT64_MIN;
        long long hi = type == ZNValueTypeI32 ? INT32_MAX : INT64_MAX;
        if (minValue) lo = MAX(lo, minValue.longLongValue);
        if (maxValue) hi = MIN(hi, maxValue.longLongValue);
        if (v < lo || v > hi) { if (error) *error = [NSString stringWithFormat:@"超出 %@ 范围", ZNValueTypeName(type)]; return nil; }
        long long step = MAX(1LL, stepValue.longLongValue);
        if (step > 1) v = lo + llround((double)(v - lo) / (double)step) * step;
        return [NSString stringWithFormat:@"%lld", v];
    }

    if (type == ZNValueTypeU32 || type == ZNValueTypeU64) {
        unsigned long long v = 0;
        if (!ZNParseUnsigned(text, &v)) { if (error) *error = @"不是有效无符号整数"; return nil; }
        unsigned long long hi = type == ZNValueTypeU32 ? UINT32_MAX : UINT64_MAX;
        unsigned long long lo = minValue ? minValue.unsignedLongLongValue : 0;
        if (maxValue) hi = MIN(hi, maxValue.unsignedLongLongValue);
        if (v < lo || v > hi) { if (error) *error = [NSString stringWithFormat:@"超出 %@ 范围", ZNValueTypeName(type)]; return nil; }
        unsigned long long step = MAX(1ULL, stepValue.unsignedLongLongValue);
        if (step > 1) v = lo + ((v - lo + step / 2) / step) * step;
    ...
    double lo = minValue ? minValue.doubleValue : -DBL_MAX;
    double hi = maxValue ? maxValue.doubleValue : DBL_MAX;
    if (v < lo || v > hi) { if (error) *error = [NSString stringWithFormat:@"超出 %@ 范围", ZNValueTypeName(type)]; return nil; }
    double step = stepValue.doubleValue;
    if (step > 0 && isfinite(step)) v = lo + round((v - lo) / step) * step;
    if (type == ZNValueTypeF32) return [NSString stringWithFormat:@"%.9g", (float)v];
    return [NSString stringWithFormat:@"%.17g", v];
}
