...
static NSData *ZNRABSerialize(NSArray<ZNRuntimeMethodAction *> *actions, NSString **error) {
    ...
    for(NSUInteger i=0;i<actions.count;i++){
        ...
        uint32_t argument0Offset=0,signatureOffset=0,argumentVectorOffset=0,controlsOffset=0,chainOffset=0; NSString *stringError=nil;
...
static void ZNRABUpdateBuildReport(NSArray<NSString *> *builderOutputs,NSArray<ZNRuntimeMethodAction *> *actions,NSUInteger tableBytes){
    ...
    object[@"runtimeMethodCall"]=@{@"format":@"com.zonoe.runtime-action/v1",@"version":@1,@"storage":@"__ZNDATA/__zndata after Static Dispatch table",@"staticEntryABIPreserved":@YES,@"runtimeEntrySize":@(sizeof(ZNRuntimeMethodCallEntry)),@"typedArgumentMaxCount":@(ZN_RUNTIME_ACTION_MAX_ARGUMENTS),@"supportedArgumentRange":@"0-8",@"signatureEncoding":@"U+001F parameter types via reserved[1]",@"argumentVectorEncoding":@"UTF-8 JSON array via reserved[2]",@"argumentControlsEncoding":@"UTF-8 JSON via reserved[3]",@"immediateChainEncoding":@"UTF-8 JSON via reserved[4]",@"count":@(actions.count),@"bytes":@(tableBytes),@"actions":items};
