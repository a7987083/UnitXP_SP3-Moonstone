...
static BOOL ZN43Read(uintptr_t address, NSUInteger length, NSData **outData, NSString **error) {
    ...
    if (kr != KERN_SUCCESS || readSize != length) {
        if (error) *error = [NSString stringWithFormat:@"读取失败：%s (%d)，read=%llu/%lu",
                             mach_error_string(kr), kr, (unsigned long long)readSize, (unsigned long)length];
        return NO;
    }
...
static BOOL ZN43WriteTransition(uintptr_t address,
    ...
    if (!writeVerified) {
        if (rollback.length == target.length) memcpy((void *)address, rollback.bytes, rollback.length);
...
- (BOOL)validate:(NSString **)error {
    @synchronized (self) {
        ...
        self.protectionDescription = [NSString stringWithFormat:@"%@  max=%@",
                                      ZN43ProtectionString(region.protection), ZN43ProtectionString(region.maxProtection)];
...
- (BOOL)applyTemporary:(NSString **)error {
    @synchronized (self) {
        ...
        NSData *now = nil;
        if (!ZN43Read(self.runtimeAddress, self.patchBytes.length, &now, &e)) { self.lastResult = e; if (error) *error = e; return NO; }
        self.currentBytes = now;
        if ([now isEqualToData:self.patchBytes]) {
            self.applied = YES;
        ...
        if (!capable) {
            self.lastResult = [NSString stringWithFormat:@"Executable Page Probe 未通过，拒绝触碰真实目标：%@", probe.lastResult ?: @"unsupported"];
            if (error) *error = self.lastResult;
            [[ZNRuntimeLogger sharedLogger] log:[NSString stringWithFormat:@"[runtime-validate] %@", self.lastResult]];
            return NO;
        }
        ...
        ZN43RegionInfo region = {};
        if (!ZN43QueryRegion(self.runtimeAddress, self.patchBytes.length, &region, &e)) { self.lastResult = e; if (error) *error = e; return NO; }
        if (!ZN43WriteTransition(self.runtimeAddress, self.patchBytes, self.capturedOriginalBytes, region.protection, &e)) {
            self.applied = NO;
            ZN43Read(self.runtimeAddress, self.patchBytes.length, &_currentBytes, NULL);
...
- (BOOL)restoreOriginal:(NSString **)error {
    @synchronized (self) {
        ...
        ZN43RegionInfo region = {};
        if (!ZN43QueryRegion(self.runtimeAddress, self.patchBytes.length, &region, &e)) { self.lastResult = e; if (error) *error = e; return NO; }
        if (!ZN43WriteTransition(self.runtimeAddress, self.capturedOriginalBytes, self.patchBytes, region.protection, &e)) {
...
- (NSArray<NSString *> *)diagnosticLines {
    @synchronized (self) {
        ...
        return @[
            ...
            [NSString stringWithFormat:@"Offset：0x%llX    Runtime：%@", self.rva,
