...
static void initlog(void){char*h=getenv("HOME");if(!h)return;strcpy(gLogPath,h);strcpy(gLogPath+strlen(gLogPath),"/Documents/HFAMap_Learn.log");logf("[HFALearn UI v1.8.3 KeyLoaderTrace] loaded\n");}
...
id p=M0(id,(id)vc,"alloc");p=M1(id,p,"initWithFrame:",CGRect,((CGRect){{70,90},{330,250}}));M1(void,p,"setBackgroundColor:",id,color(.055,.06,.075,.97));ly=M0(id,p,"layer");if(ly)M1(void,ly,"setCornerRadius:",double,14);if(pc){id pg=M0(id,(id)pc,"alloc");pg=M2(id,pg,"initWithTarget:action:",id,gTarget,SEL,sel_registerName("panelpan:"));M1(void,pg,"setCancelsTouchesInView:",BOOL,0);M1(void,p,"addGestureRecognizer:",id,pg);}id h=mklabel((CGRect){{14,10},{302,52}},18);settext(h,"HFAMap v1.8.3 Key Loader Trace");M1(void,p,"addSubview:",id,h);id a=M0(id,(id)bc,"alloc");a=M1(id,a,"initWithFrame:",CGRect,((CGRect){{14,76},{302,44}}));M2(void,a,"setTitle:forState:",id,ns("Auto Detect / Full Scan"),u64,0);M1(void,a,"setBackgroundColor:",id,color(.18,.32,.62,1));M3(void,a,"addTarget:action:forControlEvents:",id,gTarget,SEL,sel_registerName("attach:"),u64,TOUCHUP);M1(void,p,"addSubview:",id,a);gStatus=mklabel((CGRect){{14,136},{302,94}},11);settext(gStatus,"Open the game menu, then press Full Scan.\nThe native key loader is traced before web authorization.");M1(void,p,"addSubview:",id,gStatus);M1(void,p,"setHidden:",BOOL,1);M1(void,w,"addSubview:",id,p);gPanel=p;gWindow=w;gMade=1;}
