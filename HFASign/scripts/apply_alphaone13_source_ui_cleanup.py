...
# 2. Remove external automatic source recognition routes.
...
# 3. Disable clipboard source detection/prompt.
...
# 5. Unlock code entry -> UIKit UIAlertController.
...
# 6. Remove software-source auto-add documentation from URL Scheme page.
...
struct URLSchemeView: View {
    var body: some View {
        List {
            ...
            Section("软件源") {
                Text("软件源只能在软件源页面通过 + 按钮手动添加；不扫描剪贴板，也不把外部链接自动识别为软件源。")
                    .font(.footnote).foregroundStyle(.secondary)
            }
        }
...
for path in (source_path, app_path, loader_path, sources_path, unlock_path, scheme_path, project_path, plist_path):
    clean_trailing_whitespace(path)

print("alphaone13 source/UI cleanup transform applied")
