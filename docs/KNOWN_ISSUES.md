...
| P0 | alpha12/13 签名页整体无法点击 | alpha14 移除资源根层原生分页 `TabView`，恢复单页条件承载并保留手势分页 | `LibraryView.swift`、`SigningView.swift` | 静态已修，待真机验证 |
...
| P1 | 不同第三方 App 分享导入兼容性不一致 | alpha14 增加 Framework/Bundle/directory UTI 和扩展名识别；来源 App 的 security-scoped/URL 生命周期仍需矩阵验证 | `Info.plist`、`SharedImportCoordinator`、`FR`、`FeatherApp` | 静态已修，需来源矩阵 |
...
| P2 | `BUILD_ERROR.txt` 留在成功分支 | 这是历史失败诊断，不代表当前 alpha14 失败 | `HFASign/BUILD_ERROR.txt` | 文档说明即可 |
...
- `macos-26` GitHub Actions Run 33861862805：alpha14 成功。
- Release iphoneos generic build：成功。
- ldid 处理 App 主二进制和 Frameworks：成功。
- TrollStore IPA 打包：成功。
- 元数据检查通过：显示名 `zonoe`、Bundle ID `com.hfa.sign`、版本 `3.0.0`、文件导入 UTI `public.data` 和 `com.apple.framework`。
...
- alpha14 IPA SHA-256：`cea1f6e4ba0342b053d9882ef3b9e64d6601a716e50e604efcf381666670338c`。
...
- 不能宣称 alpha14 所有签名页交互、真实证书安装和高级签名组合已经真机逐项通过。
...
| alpha14 首轮 | `LibraryView._reloadResourceFiles()` 的高阶链式表达式触发 Swift 类型检查超时 | 改为显式数组、循环和排序；Run 33861862805 成功 |
