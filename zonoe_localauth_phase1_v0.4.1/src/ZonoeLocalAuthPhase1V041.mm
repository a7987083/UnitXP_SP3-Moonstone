...
static NSString *LogPath(void) {
    ...
    return [docs stringByAppendingPathComponent:@"ZonoeLocalAuth_Phase1_v0.4.1.log"];
}
...
static BOOL HookDeclaredMethodOnly(Class cls, SEL sel, IMP replacement) {
    ...
    IMP old = method_getImplementation(declared);
    if (!old || old == replacement) return NO;
    StoreOrig(cls, sel, old);
    method_setImplementation(declared, replacement);
    ZLog(@"[HOOK] declared-only class=%s sel=%s old=%p new=%p", class_getName(cls), sel_getName(sel), old, replacement);
    return YES;
}
...
static void InstallNSURLSessionHooksSafely(void) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        BOOL changed = NO;
        changed |= HookDeclaredMethodOnly(cls, @selector(dataTaskWithRequest:completionHandler:), (IMP)ProbeDataTaskReqCompletion);
        changed |= HookDeclaredMethodOnly(cls, @selector(dataTaskWithURL:completionHandler:), (IMP)ProbeDataTaskURLCompletion);
        changed |= HookDeclaredMethodOnly(cls, @selector(dataTaskWithRequest:), (IMP)ProbeDataTaskReq);
        changed |= HookDeclaredMethodOnly(cls, @selector(dataTaskWithURL:), (IMP)ProbeDataTaskURL);
        changed |= HookDeclaredMethodOnly(cls, @selector(uploadTaskWithRequest:fromData:completionHandler:), (IMP)ProbeUploadData);
        changed |= HookDeclaredMethodOnly(cls, @selector(uploadTaskWithRequest:fromFile:completionHandler:), (IMP)ProbeUploadFile);
        if (changed) changedClasses++;
    }
    free(classes);
    ZLog(@"[SCAN] safe declared-only NSURLSession pass changedClasses=%d", changedClasses);
}
...
static void DumpPandoraSelectors(void) {
    ...
    dispatch_once(&once, ^{
        ...
        for (uint32_t i = 0; i < imageCount; ++i) {
            ...
            for (unsigned int c = 0; c < classCount && total < 400; ++c) {
                Class cls = objc_getClass(names[c]);
                if (!cls) continue;
                unsigned int mc = 0;
                Method *methods = class_copyMethodList(cls, &mc);
                for (unsigned int m = 0; m < mc && total < 400; ++m) {
...
static NSInteger ChannelLoginProbe(id self, SEL _cmd) {
    ...
    InstallNSURLSessionHooksSafely();
    DumpPandoraSelectors();
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_global_queue(QOS_CLASS_UTILITY,0), ^{ InstallNSURLSessionHooksSafely(); });
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_global_queue(QOS_CLASS_UTILITY,0), ^{ InstallNSURLSessionHooksSafely(); });
...
static void Install(void) {
    @autoreleasepool {
        ...
        if (!gOrig) gOrig = [NSMutableDictionary dictionary];
        os_unfair_lock_unlock(&gLock);
        ZLog(@"=== ZonoeLocalAuth Phase1 v0.4.1 Safe Auth Route Probe start ===");
        ZLog(@"[NOTE] no network swizzling during constructor; declared-only NSURLSession hooks are installed only after login begins");
        LogImages();
        for (int i = 0; i < 160; ++i) {
        ...
        ZLog(@"[READY] safe auth route probe installed");
    }
}
