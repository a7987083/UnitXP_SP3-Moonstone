...
#define JCG4_VERSION @"JSONCapture g4 中文监控 + 手机JSON恢复"
...
static NSString *JCG4PartialJSONDir(void) { return [gRootPath stringByAppendingPathComponent:@"recovered_partial"]; }
...
static BOOL JCG4CaptureBusy(void) {
    if (gDiskWorkerActive) return YES;
    if (gDiskBundleCandidates > (gDiskBundlesLoaded + gDiskBundlesFailed)) return YES;
    BOOL pending = NO;
    pthread_mutex_lock(&gDiskLock);
    pending = gDiskPendingPaths.count > 0;
...
static void JCG4RunRecoveryIfNeeded(void) {
    ...
    dispatch_async(gJCG4RecoveryQueue, ^{
        @autoreleasepool {
            ...
            if ([stats[@"yielded_for_capture"] boolValue]) {
                gJCG4IncrementalRecoverRequested = YES;
                JCG4SetLastEvent(@"发现抓取任务，JSON恢复已让路");
            } else {
...
static void JCG4WorkerTick(void) {
    ...
    if (!JCG4CaptureBusy() && !gJCG4Decrypting && !gJCG4Recovering) gJCG4IncrementalRecoverRequested = YES;
    JCG4ScheduleWorkers();
}
...
- (void)viewDidLoad {
    ...
    title.text = @"JSONCapture g4 中文监控";
...
- (void)dragBubble:(UIPanGestureRecognizer *)pan {
    CGPoint t=[pan translationInView:self.view];CGPoint c=self.bubble.center;c.x+=t.x;c.y+=t.y;CGFloat r=29;c.x=MAX(r+4,MIN(self.view.bounds.size.width-r-4,c.x));c.y=MAX(r+4,MIN(self.view.bounds.size.height-r-4,c.y));self.bubble.center=c;[pan setTranslation:CGPointZero inView:self.view];
...
- (void)toggleScanning {
    if(gRunning){gRunning=NO;JCG4SetLastEvent(@"扫描已暂停，后台恢复队列也会等待");}
    else {gRunning=YES;JCG4SetLastEvent(@"扫描已继续");JCG2ScheduleLoadedWatch();JCG2ScheduleDiskWatch();JCG3ScheduleLogPoll();}
}

- (void)forceFullScan {
    if(JCG4CaptureBusy()){JCG4SetLastEvent(@"已有抓取任务，强制全扫稍后再试");return;}
...
- (void)redecryptAll { gJCG4ForceDecryptRequested=YES;JCG4SetLastEvent(@"重新解密已排队；抓取任务优先");JCG4RunDecryptIfNeeded(); }
- (void)rerecoverAll { ODLRResetRecoveryIndex(gRootPath);gJCG4ForceRecoverRequested=YES;JCG4SetLastEvent(@"重新恢复JSON已排队；抓取任务优先");JCG4RunRecoveryIfNeeded(); }
...
static void JCG4InstallUI(void) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if(@available(iOS 13.0,*)){UIWindowScene *scene=JCG4ForegroundScene();if(scene)gJCG4Window=[[UIWindow alloc]initWithWindowScene:scene];else gJCG4Window=[[UIWindow alloc]initWithFrame:bounds];}
        else gJCG4Window=[[UIWindow alloc]initWithFrame:bounds];
        ...
        JCG4SetLastEvent(@"g4 中文监控已启动");
    });
}

__attribute__((constructor)) static void JCG4Entry(void) {
    @autoreleasepool {
        gJCG4DecryptQueue=dispatch_queue_create("com.openai.jsoncapture.g4.decrypt",DISPATCH_QUEUE_SERIAL);
        gJCG4RecoveryQueue=dispatch_queue_create("com.openai.jsoncapture.g4.recovery",DISPATCH_QUEUE_SERIAL);
        ...
        JCG2Log([NSString stringWithFormat:@"%@ loaded; capture priority > re-decrypt > static JSON recovery",JCG4_VERSION]);
    }
}
