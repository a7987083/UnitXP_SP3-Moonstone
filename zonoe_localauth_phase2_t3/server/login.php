...
/* Fill these values only on the server. Do not commit real credentials. */
$DB_HOST = '127.0.0.1';
$DB_PORT = 3306;
...
/* Dedicated T3 test account, configured only on the server. */
$T3_TEST_ACCOUNT = 'CHANGE_ME';
$T3_TEST_PASSWORD = 'CHANGE_ME';
...
/* Safe self-test mode: client sends no account credentials. */
if (isset($input['mode']) && $input['mode'] === 'server_self_test') {
    if ($T3_TEST_ACCOUNT === 'CHANGE_ME' || $T3_TEST_PASSWORD === 'CHANGE_ME') {
        out_json(false, 'T3_TEST_ACCOUNT_REQUIRED', array(), 500);
    }
    $username = $T3_TEST_ACCOUNT;
    $password = $T3_TEST_PASSWORD;
}
