...
if (strlen($username) > 64 || strlen($password) > 64 || strlen($channel) > 64) out_json(false, 'ACCOUNT_OR_PASSWORD_INVALID');
...
if (!$lockResult) out_json(false, 'ID_LOCK_FAILED', array(), 500);
$lockRow = $lockResult->fetch_assoc();
if (!isset($lockRow['locked']) || (int)$lockRow['locked'] !== 1) out_json(false, 'ID_LOCK_TIMEOUT', array(), 503);

$stmt = $mysqli->prepare('SELECT id FROM t_tb_account WHERE account = ? LIMIT 1');
if (!$stmt) { release_id_lock($mysqli); out_json(false, 'DB_PREPARE_FAILED', array(), 500); }
$stmt->bind_param('s', $username);
if (!$stmt->execute()) { release_id_lock($mysqli); out_json(false, 'DB_QUERY_FAILED', array(), 500); }
$stmt->store_result();
if ($stmt->num_rows > 0) { release_id_lock($mysqli); out_json(false, 'ACCOUNT_EXISTS'); }
$stmt->close();

$idResult = $mysqli->query('SELECT MAX(id) AS max_id FROM t_tb_account');
if (!$idResult) { release_id_lock($mysqli); out_json(false, 'ID_QUERY_FAILED', array(), 500); }
$idRow = $idResult->fetch_assoc();
$maxId = isset($idRow['max_id']) && $idRow['max_id'] !== null ? (int)$idRow['max_id'] : 1048576;
$nextId = $maxId + 1;
if ($nextId <= 0 || $nextId > 4294967295) { release_id_lock($mysqli); out_json(false, 'ID_RANGE_EXHAUSTED', array(), 500); }
...
if (!$insert) { release_id_lock($mysqli); out_json(false, 'DB_PREPARE_FAILED', array(), 500); }
...
$insert->close();
release_id_lock($mysqli);
out_json(true, 'REGISTER_OK', array('account_id' => (int)$nextId, 'account' => $username, 'channel' => $channelValue, 'last_server' => null));
