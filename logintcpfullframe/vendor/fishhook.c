...
static void perform_rebinding_with_section(struct rebindings_entry *rebindings,
  ...
  for (uint i = 0; i < section->size / sizeof(void *); i++) {
    ...
    bool valid = symbol_name[0] && symbol_name[1];
    struct rebindings_entry *cur = rebindings;
    while (cur) {
      for (uint j = 0; j < cur->rebindings_nel; j++) {
        if (valid && strcmp(&symbol_name[1], cur->rebindings[j].name) == 0) {
          if (cur->rebindings[j].replaced && bindings[i] != cur->rebindings[j].replacement)
            *(cur->rebindings[j].replaced) = bindings[i];
          kern_return_t err = vm_protect(mach_task_self(), (uintptr_t)bindings,
                                         section->size, 0,
                                         VM_PROT_READ | VM_PROT_WRITE | VM_PROT_COPY);
          if (err == KERN_SUCCESS) bindings[i] = cur->rebindings[j].replacement;
          goto symbol_loop;
        }
      }
      cur = cur->next;
...
int rebind_symbols_image(void *header, intptr_t slide, struct rebinding rebindings[], size_t nel) {
  struct rebindings_entry *head = NULL;
  int retval = prepend_rebindings(&head, rebindings, nel);
  rebind_symbols_for_image(head, (const struct mach_header *)header, slide);
  if (head) free(head->rebindings);
  free(head);
  return retval;
}

int rebind_symbols(struct rebinding rebindings[], size_t nel) {
  int retval = prepend_rebindings(&_rebindings_head, rebindings, nel);
